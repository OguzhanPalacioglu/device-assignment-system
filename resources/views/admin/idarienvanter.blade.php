<x-app-layout>
    <x-slot name="header">
        İDARİ KADRO ENVANTER LİSTESİ
    </x-slot>

  <div class="card mb-1">
        <div class="card-body">
            <div class="card-header text-center"><b>İDARİ ZİMMETTE OLMAYANLAR</b></div>
                <div class="m-2">
                    <h5 class="card-title float-right"><a href="{{route('envanter.create')}}" class="btn btn-sm btn-primary"> Envanter Ekle </a> </h5>
                </div>
    @if ($idarizimmetsiz->isEmpty())
         İdari Kadro Envanterinde Zimmette Olmayan Eşya Bulunamadı.
    @else
    ...
    <div class="card">
        <div class="card-body">
            <div class="card-header text-center"><b>İDARİ ZİMMETLİ ENVANTER LİSTESİ</b></div>
            <table class="table table-striped table-bordered table-sm">
                <thead>
                    <tr>
                        <th scope="col">Ürünün Çeşidi</th>
                ...
                <tbody>
                   ...
                   @foreach ($envantergoster->envanter as $a)
                      <tr>
                        ...
                        <td>{{$a->seri_no}}</td>
                        <td>{{$a->okul}}</td>
                        <td style="width:210px"><a href="{{route('kisiler.edit',$envantergoster->id)}}" class="btn btn-sm btn-vekla d-flex justify-content-center m-1">{{$envantergoster->ad_soyad}}</a></td>
